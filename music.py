...
class Music(commands.Cog):
    ...
    async def slowplay(self, ctx, *, url):
        ...
        async with ctx.typing():
            ...
            ctx.voice_client.play(player, after=lambda e: print("Lejátszó hiba: %s" % e) if e else None)

        await ctx.send("Jelenleg lejátszás alatt(**/2/**): {}".format(player.title))
    ...
    async def play(self, ctx, *, url):
        ...
        await ctx.send("Jelenleg lejátszás alatt: {}".format(player.title))
    ...
    async def volume(self, ctx, volume: int):
        ...
        if ctx.voice_client is None:
            return await ctx.send("Nem vagy hangcsatornában.")

        ctx.voice_client.source.volume = volume / 100
        await ctx.send("Hangerő módosítva erre: {}%".format(volume))
    ...
    async def ensure_voice(self, ctx):
        if ctx.voice_client is None:
            if ctx.author.voice:
                ...
                raise commands.CommandError("nem csatlakozott hangcsatornához.")
